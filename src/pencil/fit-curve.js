...
function norm_renamed(p) {
  return Math.sqrt(p.x ** 2 + p.y ** 2);
}

function squaredNorm(p) {
  return p.x ** 2 + p.y ** 2;
}

function add_renamed(p0, p1) {
  return { x: p0.x + p1.x, y: p0.y + p1.y };
}

function addN(...points) {
  return {
...
function sub_renamed(p0, p1) {
  return { x: p0.x - p1.x, y: p0.y - p1.y };
}

function scalarMul(p, a) {
  return { x: p.x * a, y: p.y * a };
}

function scalarDiv(p, a) {
  return { x: p.x / a, y: p.y / a };
}

function normalize_renamed(p) {
  const len = norm_renamed(p);
  return len === 0 ? p : scalarDiv(p, len);
}

function dot_renamed(p0, p1) {
 ...
 */
export function fitCurve(points, maxError) {
  if (points.length < 2) {
    return [];
  }
...
function fitCubic(points, leftTangent, rightTangent, error) {
  ...
  if (points.length === 2) {
    ...
    return [
      [
        points[0],
        add_renamed(points[0], scalarMul(leftTangent, dist)),
        add_renamed(points[1], scalarMul(rightTangent, dist)),
        points[1],
      ],
    ];
  }
  ...
  let centerVector = sub_renamed(points[splitPoint - 1], points[splitPoint + 1]);
  ...
  if (centerVector.x === 0 && centerVector.y === 0) {
    ...
    const { x, y } = sub_renamed(points[splitPoint - 1], points[splitPoint]);
    centerVector = { x: -y, y: x };
  }
  const toCenterTangent = normalize_renamed(centerVector);
...
function generateBezier(points, parameters, leftTangent, rightTangent) {
  ...
  const bezCurve = [firstPoint, zeroPoint, zeroPoint, lastPoint];
  //Compute the A's
  const A = parameters.map((u) => {
    const ux = 1 - u;
    return [scalarMul(leftTangent, 3 * u * (ux * ux)), scalarMul(rightTangent, 3 * ux * (u * u))];
  });
  ...
  for (const [i, u] of parameters.entries()) {
    const [a, b] = A[i];
    C[0][0] += dot_renamed(a, a);
    C[0][1] += dot_renamed(a, b);
    C[1][0] += dot_renamed(a, b);
    C[1][1] += dot_renamed(b, b);
    const tmp = sub_renamed(points[i], bezierQ([firstPoint, firstPoint, lastPoint, lastPoint], u));
    X[0] += dot_renamed(a, tmp);
    X[1] += dot_renamed(b, tmp);
  }
  ...
  const segLength = norm_renamed(sub_renamed(firstPoint, lastPoint));
  const epsilon = 1.0e-6 * segLength;
  if (alpha_l < epsilon || alpha_r < epsilon) {
    //Fall back on standard (probably inaccurate) formula, and subdivide further if needed.
    bezCurve[1] = add_renamed(firstPoint, scalarMul(leftTangent, segLength / 3.0));
    bezCurve[2] = add_renamed(lastPoint, scalarMul(rightTangent, segLength / 3.0));
  } else {
    //First and last control points of the Bezier curve are
    //positioned exactly at the first and last data points
    //Control points 1 and 2 are positioned an alpha distance out
    //on the tangent vectors, left and right, respectively
    bezCurve[1] = add_renamed(firstPoint, scalarMul(leftTangent, alpha_l));
    bezCurve[2] = add_renamed(lastPoint, scalarMul(rightTangent, alpha_r));
  }
  return bezCurve;
}
/**
...
function newtonRaphsonRootFind(bez, point, u) {
  /*
      ...
      */
  const d = sub_renamed(bezierQ(bez, u), point);
  const qprime = bezierQPrime(bez, u);
  const numerator = dot_renamed(d, qprime);
  const denominator = squaredNorm(qprime) + 2 * dot_renamed(d, bezierQPrimePrime(bez, u));
  if (denominator === 0) {
    return u;
  } else {
...
function chordLengthParameterize(points) {
  ...
  let prevP;
  points.forEach((p, i) => {
    currU = i > 0 ? prevU + norm_renamed(sub_renamed(p, prevP)) : 0;
    u.push(currU);
    prevU = currU;
    prevP = p;
  });
...
function computeMaxError(points, bez, parameters) {
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    const v = sub_renamed(bezierQ(bez, t), point);
    // current error
...
function mapTtoRelativeDistances(bez, B_parts) {
  ...
  for (let i = 1; i <= B_parts; i++) {
    ...
    sumLen += norm_renamed(sub_renamed(B_t_curr, B_t_prev));
...
function createTangent(pointA, pointB) {
  return normalize_renamed(sub_renamed(pointA, pointB));
}
//Evaluates cubic bezier at t, return point
function bezierQ(ctrlPoly, t) {
  const tx = 1.0 - t;
...
function bezierQPrime(ctrlPoly, t) {
  const tx = 1.0 - t;
  const pA = scalarMul(sub_renamed(ctrlPoly[1], ctrlPoly[0]), 3 * tx ** 2);
  const pB = scalarMul(sub_renamed(ctrlPoly[2], ctrlPoly[1]), 6 * tx * t);
  const pC = scalarMul(sub_renamed(ctrlPoly[3], ctrlPoly[2]), 3 * t ** 2);
  return addN(pA, pB, pC);
}
//Evaluates cubic bezier second derivative at t, return point
function bezierQPrimePrime(ctrlPoly, t) {
  return add_renamed(
    scalarMul(add_renamed(sub_renamed(ctrlPoly[2], scalarMul(ctrlPoly[1], 2)), ctrlPoly[0]), 6 * (1.0 - t)),
    scalarMul(add_renamed(sub_renamed(ctrlPoly[3], scalarMul(ctrlPoly[2], 2)), ctrlPoly[1]), 6 * t)
  );
}
