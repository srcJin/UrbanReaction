...
export function ProfiledContourGeometry(profileShape, contour, contourClosed, openEnded) {
  ...
  let coords = [];
  coords.length = (2 * fullProfileGeometry.attributes.position.array.length) / 3;

  fullProfileGeometry.rotateX(Math.PI * 0.5);
  fullProfileGeometry.setAttribute('uv', new THREE.Float32BufferAttribute(coords, 2));

  fullProfileGeometry.attributes.uv.needsUpdate = true;
