...
console.log("-------------------");
console.log("water_generator.ts running!");
/**
...
export default class WaterGenerator extends StreamlineGenerator {
    constructor(integrator, origin, worldDimensions, params, tensorField) {
        ...
        this._seaPolygon = []; // Uses screen rectangle and simplified road
        this._riverPolygon = []; // Simplified
    ...
    createRiver() {
        ...
        const road1 = expandedNoisy.filter(v => !PolygonUtil.insidePolygon(v, this._seaPolygon)
            && !this.vectorOffScreen(v)
            && PolygonUtil.insidePolygon(v, riverSplitPoly));
        const road1Simple = this.simplifyStreamline(road1);
        const road2 = expandedNoisy.filter(v => !PolygonUtil.insidePolygon(v, this._seaPolygon)
            && !this.vectorOffScreen(v)
            && !PolygonUtil.insidePolygon(v, riverSplitPoly));
